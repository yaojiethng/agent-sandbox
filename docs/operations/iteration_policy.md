...
| **7 — Pre-close verification** | confirmed | Implementation complete | Present pre-close summary: AC status per criterion, proposed compaction entries, and file-level change summary per [`handover_policy.md`](handover_policy.md#at-pre-close-verification-step-7). Wait for explicit release. | Operator confirmed against AC and compaction text. |
| **Gate 3** | always | Pre-close verified | No close until operator releases. | Explicit release received. |
| **8–9 — Close session** | always | Gate 3 released | Apply approved compaction to roadmap. Run Trigger B if applicable per [`roadmap_policy.md`](roadmap_policy.md#session-close-steps-8-9). Verify all in-scope architecture and concepts docs describe the system as built — resolve divergences or record as explicit deferrals blocking Trigger B. Close handover and seed next session per [`handover_policy.md`](handover_policy.md#at-session-close-steps-8-9). | Roadmap updated. Handover closed. No doc divergences without explicit deferral. Next session section actionable. |
