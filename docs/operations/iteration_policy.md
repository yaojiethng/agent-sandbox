...
**Propagation replay:** for any session that touched multiple files under a shared rule or naming convention, the pre-close summary must include a row-by-row comparison of every file that was planned to receive the change against the Completed this session table:
