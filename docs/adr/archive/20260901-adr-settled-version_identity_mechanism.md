...
4. **Freshness signal retired.** `[IMAGE_STALE]` / `record_image_stale` /
   `image_is_stale` list-time usage is removed (kills the 2xN docker-inspect
...
- **`resume LIST` slowness removed** -- the 2xN docker-inspect staleness scan is
  gone.
