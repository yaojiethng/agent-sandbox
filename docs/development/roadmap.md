...
| [M1.3 — Invocation Cleanup & Onboarding Workflow](#m13--invocation-cleanup--onboarding-workflow) | Not started |
...
The per-project conf file (`opencode.<machine>.conf`) is removed. Project identity and paths move into the project-side `Makefile`; machine-specific variables (`SERVE_PORT`, `OPENCODE_SERVER_PASSWORD`) stay in a `.env` alongside it. `start_agent.sh` accepts named flags; `execution_model.md` documents the new invocation model. The operator-facing onboarding experience remains to be defined and documented.

- [ ] Write `scripts/agent-sandbox.sh` dispatch wrapper with `@@AGENT_SANDBOX_REPO@@` placeholder; subcommands: `start`, `dry-run`, `build`, `apply`, `apply-branch`; build-if-missing and `--rebuild` logic for `start`/`dry-run`
- [ ] Write `providers/opencode/build_agent.sh`; pure image build, takes `--name`, `--root`, `--no-cache`
- [ ] Refactor `start_agent.sh`: remove `--build` flag and build logic; hard-fail if image missing
- [ ] Add `install` and `uninstall` targets to agent-sandbox `Makefile` (`PREFIX` defaulting to `/usr/local/bin`)
- [ ] Define the onboarding workflow: repo setup, WSL configuration, first project registration
- [ ] Agree on scope and structure of `docs/operations/quickstart.md`
