...
- **Resume a session:** `make resume SESSION_ID=<id>` resumes that session's git state and volume. `make resume LIST=1` lists resumable sessions (registry-truth `fresh`/`stale`/`unknown` sandbox-staleness + image-truth `container-sig` staleness columns, capped at 10 rows); `make resume INTERACTIVE=1` picks + confirms, marking `[STALE]`/`[IMG-STALE]` sessions; `PROVIDER=<n>` filters either by provider.
