...
- **Resume a session:** `make resume SESSION_ID=<id>` resumes that session's git state and volume. `make resume LIST=1` lists resumable sessions in an enriched table (`SESSION_ID | PROVIDER | STARTED | BRANCH | LAST_USED`, relative times, newest first, capped at 10 rows), flagging staleness as `[SANDBOX_STALE]`/`[IMAGE_STALE]` warning labels; `make resume INTERACTIVE=1` picks + confirms (same markers); `PROVIDER=<n>` filters either by provider.
