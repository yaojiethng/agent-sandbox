...
- **Resume a session:** `make resume SESSION_ID=<id>` resumes that session's git state and volume. `make resume LIST=1` lists resumable sessions (with a registry-truth `fresh`/`stale`/`unknown` sandbox-staleness column); `make resume INTERACTIVE=1` picks + confirms; `PROVIDER=<n>` filters either by provider.
