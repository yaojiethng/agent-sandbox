...
- First run materializes the worktree via the shared snapshot primitive as above, then records the delivery-history mode in the worktree config. Resume stages nothing — the registry record and worktree init marker are authoritative. `SESSION_STATE` is written into the worktree `.git` (doubles as the init marker).
