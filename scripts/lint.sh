...
# cannot run.
#
# All gates always run, so a failure in one never hides the others. The exit
# code is a verdict, not a count: 0 = all clean, 1 = at least one failed (see
...
bash "$REPO_ROOT/scripts/check_lib_contract.sh" || rc=1
