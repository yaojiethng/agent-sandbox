...
#   Makefile                      --  from template (project identity read from .env)
...
_run_refresh() {
  ...
  cp "$TEMPLATES/Makefile.template" "$SANDBOX_DIR/Makefile"
  ...
  if [[ ! -f "$ENV_FILE" ]]; then
    ...
    local _esc_pd _esc_sd _esc_pn
    _esc_pd=${PROJECT_DIR//&/\\&}
    _esc_sd=${SANDBOX_DIR//&/\\&}
    _esc_pn=${PROJECT_NAME//&/\\&}
    sed -i \
      ...
      "$ENV_FILE"
    # Migrate a .env onboarded before PROJECT_NAME was stored there: the name
    # is now read from .env, so an old sandbox must gain the line here.
    if grep -q '^PROJECT_NAME=' "$ENV_FILE"; then
      sed -i "s/^PROJECT_NAME=.*/PROJECT_NAME=${_esc_pn}/" "$ENV_FILE"
    else
      sed -i "\|^SANDBOX_DIR=.*|a PROJECT_NAME=${_esc_pn}" "$ENV_FILE"
    fi
    echo "  Updated: .env (project name + paths + template versions)"
  fi
...
# --- Project identity (set at onboard time) ---
