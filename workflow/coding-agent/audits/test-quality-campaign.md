...
Scope: tests and the test runner (`scripts/run_tests.sh`). Change the runner only where a success criterion requires it. Never change product source under `src/`. If a test change exposes a product defect, do not fix it here -- record it in `FLAKY_AND_BAD_TESTS.md` and flag it. If this scope statement conflicts with a success criterion, stop and ask the operator for a ruling; do not resolve the conflict silently by changing scope.
