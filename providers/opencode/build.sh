#!/usr/bin/env bash
# providers/opencode/build.sh
# Usage:
#   ./build.sh --name=<project_name> [--no-cache]
#
# Builds the OpenCode reasoning layer images for a project:
#   1. opencode-base          — stable install layers (built once; skipped if exists and not --no-cache)
#   2. opencode-agent-<project> — provider layer inheriting from opencode-base
#
...
BASE_IMAGE_NAME="opencode-base"
IMAGE_NAME="opencode-agent-${PROJECT_NAME,,}"
BASE_DOCKERFILE="$REPO_ROOT/providers/opencode/base.Dockerfile"
PROVIDER_DOCKERFILE="$REPO_ROOT/providers/opencode/provider.Dockerfile"
