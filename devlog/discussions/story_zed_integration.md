...
How each axis value contributes to each use case. `[x]` = covers; `partial` = partial fit; `--` = does not address; `?` = pending experimental confirmation.
...
| **B0** (host shell) | -- | [x] via `make apply` / `make draft` | -- | [x] | -- | -- | -- |
| **B1** (attach reasoning TUI) | -- | -- | [x] with discipline (wait for welcome message before attach) | -- | -- | -- | -- |
...
| **C0** (`PROJECT_DIR` workspace) | [x]  | -- | -- | -- | -- | -- | -- |
| **C1** (`SANDBOX_DIR` workspace) | -- | [x]  via tasks running `make apply` | -- | [x] via task wrappers + file tree | [x] | [x] | -- |
| **C2** (container workspace) | -- | -- | -- | -- | -- | -- | [x] editor or diff view on `sandbox/` |
