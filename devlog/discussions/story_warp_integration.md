...
How each axis value contributes to each use case. `[x]` = covers; `partial` = partial fit; `--` = does not address.
...
| **B1** (attach reasoning TUI) | -- | -- | [x] subject to wait-for-welcome discipline | -- | -- | -- | -- |
...
| **C0** (`PROJECT_DIR` host workspace) | [x] | -- | -- | -- | -- | -- | -- |
| **C1** (`SANDBOX_DIR` host workspace) | -- | [x] via Workflows running `make apply` | -- | [x] via Workflows + file tree | [x] | [x] | -- |
| **D1** | -- | -- | -- | -- | -- | -- | -- |
