...
Worktree backing (agent commits landing in the host object store via `git worktree add`) is rejected — see [ADR](../../docs/adr/sandbox_delivery_model.md) and [full analysis](20260730-study-settled-worktree_rejection.md).
...
- **Two ADRs** (`sandbox_delivery_model`, `20260722-adr-settled-mount_model_axes`): labeled settled but revised within days with nothing implemented against them. Consolidated into this document.
...
- **Q7 / start design** — **two-command split** (supersedes the earlier merged `start`-wizard sketch in this entry; settled in F2 design session `20260821-02`). `make start` unconditionally starts a NEW session (no resume branch; its interactive form is a provider/config wizard). `make resume` is the resume entrypoint: it inventories the unified `.compose/<session-id>.yml` registry (both deliveries; secondary existence check per delivery) and filters it by args — `--session-id=<id>` is the direct-resume bypass (silent), `PROVIDER=` narrows, `--interactive` opens a picker+confirm, and bare `resume` prints help. Cross-command convention: fast mode = supplied args, slow mode = explicit `--interactive` flag. Freshness-on-new = container freshness (implicit rebuild downgraded by staleness detection). The flag name is `--session-id=<id>` (terminology sweep — `run`→`session`); the earlier `--run=<id>` is stale and must not survive.
...
Worktree backing is rejected. See [ADR — Worktree Backing Rejected](../../docs/adr/sandbox_delivery_model.md) and the [full investigation](20260730-study-settled-worktree_rejection.md).
...
| [`docs/adr/sandbox_delivery_model.md`](../../docs/adr/sandbox_delivery_model.md) | ADR: worktree backing rejected |
