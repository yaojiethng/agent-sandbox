...
**B. Introduce a session plan document.** A new artifact type in `devlog/` (alongside handovers) that represents the unit queue for the current sub-milestone. Persists across sessions within the sub-milestone; closed at sub-milestone close (post-close bookkeeping). Requires new policy and new tooling conventions.
