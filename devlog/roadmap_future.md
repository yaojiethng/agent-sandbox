...
### M2.6.7 — Interface Contract Compatibility (design settled; P0 + docs + P2 default-warn landed; strict flip scheduled)

**Design settled 20260919-03.** P0 landed 20260919-04 (version constant, image label, record stamps, warn-only preflight check). Doc consolidation landed 20260919-05: one interface concept doc (`docs/concepts/sandbox_host_interface.md`, renamed from the correspondence model) + one lifecycle architecture doc (`sandbox_lifecycle.md`). One version `INTERFACE_CONTRACT_VERSION` declared in `src/libs/interface_contract.sh`, stamped into tier-3 images at build (declaration 1) and into the record (`.compose` label set + `SESSION_STATE` key, declaration 2); comparison layered by surface: host<->container + record at start/resume preflight, container<->container at the agent entrypoint. P2 landed default-warn 20260919-06: flag `interface_contract_strict()` + preflight policy enforcement + agent-entrypoint `_check_container_contract`; container-sig untouched. Warn-then-strict mismatch policy with operator-released switch-over gates; container-sig rollover P0-P3, old check stripped only after the new is proven. ADR `docs/adr/interface_contract_compatibility.md` (open — mechanism landed, authoritative on the strict flip). Discussion `devlog/discussions/20260919-design-interface_contract_compatibility.md` (settled). Next scheduled iteration: flip the flag to strict and fix any errors the strict regime exposes.
