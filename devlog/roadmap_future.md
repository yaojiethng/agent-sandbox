...
### M2.6.7 — Interface Contract Compatibility (design settled; impl scheduled next)

**Design settled 20260919-03.** The deferred interface-contract thread (roadmap row 117) lands as the next iteration. One version `INTERFACE_CONTRACT_VERSION` declared in `src/libs/interface_contract.sh`, stamped into tier-3 images at build (declaration 1) and into the record (`.compose` label set + `SESSION_STATE` key, declaration 2); comparison layered by surface: host<->container + record at start/resume preflight, container<->container at the agent entrypoint. Warn-then-strict mismatch policy with operator-released switch-over gates; container-sig rollover P0-P3, old check stripped only after the new is proven. Doc consolidation: one interface concept doc (`sandbox_host_correspondence_model.md` -> `sandbox_host_interface.md`, 17-file propagation checklist) + one lifecycle architecture doc (`sandbox_lifecycle.md`). ADR `docs/adr/interface_contract_compatibility.md` (open/pending-impl). Discussion `devlog/discussions/20260919-design-interface_contract_compatibility.md` (settled).
