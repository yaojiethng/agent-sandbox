...
- [x] Pi version pinned in base Dockerfile (current: `@earendil-works/pi-coding-agent@0.84.1`)
...
**Deferred decision `20260818-02` (keep RO-mount-at-start); dependency (compose file-set mechanism) has since landed; discovery validated `20260901-13` — impl task now active on the main roadmap ("Host-side volume seed + git-enumerated tar").** A cleaner copy-in delivery mechanism exists but is not adopted yet: seed the volume host-side before `compose up` (one-shot `docker create` + `docker cp` reusing the sandbox image, or `docker run --rm` with both mounts), then bring the container up with no snapshot mount at all — fresh and resume compose files become identical and the one-time-copy-in semantics are captured exactly (staging exists only during the seed step). Serialization: git-enumerated tar; discovery tests (`scripts/manual/discovery_tar_*.sh`) confirmed lossless round-trip and parity, incl. an rsync negation-pattern exclusion leak the new mechanism fixes. Model: [`docs/concepts/copy_delivery.md`](../docs/concepts/copy_delivery.md). Subtasks, to clean up after the seeding lands:
