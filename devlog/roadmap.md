...
- [ ] **Test-authoring parity for the harness comparison** -- equalize the authoring model across branch A and branch B before the authoritative harness decision, so the comparison judges the harness rather than the authoring style. The two-branch comparison evaluates branch B at its minimal migration (bespoke bodies re-skinned as `@test` blocks, `set +e; set +T` heads kept) against branch A at the status-quo suite; the two suites are authored differently in kind (in-process shared fixtures and PASS-marker accounting versus per-`@test` subshell isolation and test-unit counting), which is a fault in the comparison methodology recorded in the study and both branch reports. The parity items are harness-independent and benefit both suites. Checklist: capture-and-assert (run the command, keep its rc and output, assert on both); assert the meaning, not the string; per-test fixture directories instead of destructive reuse of one shared fixture; explicit teardown (rm fixture, kill background processes, wait); no reliance on benign non-zero intermediates; unique descriptive labels; a sweep for order-dependence. Record each item's status per branch in the two branch reports, then re-measure both suites on equal footing (wall time, an isolation probe, final dependency count) and only then settle the harness decision.
