...
- [ ] **`start` command redesign (resumable/stale volume UX)** — **Design SETTLED (walk `20260818-02`): interactive-by-default wizard** — agent-run inventory first (copy via labels; bind-mount via registry), resume-N or new; freshness-on-new (implicit rebuild, auto-downgraded); config prefilled from the newest run; prints the full non-interactive command; `--run=<id>` resumes (absence = new); no subcommand split. Decision rows D6/A-Resolved/N2a/N4; implementation carried by the M2.6.6 delivery tasks
  - [ ] **Connect session-id → `start` resume pass-through (blocked-on for Finding 1 in `20260821-01-fix-start_agent_bugfixes.md`)** — `make start` today passes only `RESUME`/`--resume` (picker-only, no specific session); `start_agent.sh` has NO `--session-id` flag (SESSION_ID is derived, not passed in); the Makefile `start` target does not use `SESSION_ID_FLAG` (that's `stop`-only). Resuming a *specific* session directly (needed for the `make stop` resume-completion message) requires the `--run=<id>`/`--session-id=<id>` pass-through link-up from this F2 spec. Once wired, `stop.sh` teardown can print an accurate resume command for the session it stopped.
