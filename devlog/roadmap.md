...
- [ ] **test_runner_selftest.sh FATAL (raised 20260912-13)** - the test-harness self-check reports `FATAL test_runner_selftest.sh: run_test registered after test_done` on clean trees, the registration is dead code. The suite still passes; the defect is in the harness self-check, not the suites. Own dedicated fix iteration required (operator instruction); handover `20260912-13` flagged it as a pre-existing finding.
