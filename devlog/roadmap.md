...
- [ ] **Prune-command redesign (registry + shape)** — **Rule 2 CONFIRMED** (design walk `20260818-02`): at prune time, a session under the current sandbox with no matching `.compose/<run-id>.yml` is prunable — scope differs by delivery (copy → volume + containers; bind-mount → registry resources only; worktrees never touched, D8/D4). **Command SHAPE redesign DEFERRED until after the M2.6.5/M2.6.6 artifact shapes settle** (registry, worktree, locks) — requirements will be clearer then. **STALE=1 terminology REJECTED** (unclear: `=1` as boolean-true vs staleness filter; prune-all-stale vs keep-all-stale ambiguity; unknown command for "prune everything"; scope of "everything" incl. active containers undefined). Minimums for the redesign: interactive mode showing the cutoff + ask confirmation; descriptive option names; explicit scope semantics
...
- [ ] **Terminology sweep — three-phase; terms registered in `docs/concepts/terminology.md`** — mapping REVERSED from the design walk (operator, session `20260819-09`): **session** = one container lifecycle (harness unit; start→teardown; identified by `SESSION_ID` formerly `RUN_ID`; may contain zero or more iterations); **iteration** = one work cycle producing handover + commit (ops unit; the `new-session` skill→`new-iteration`). Terms registered as reserved technical terms (register + first-mention section-link convention established; policy docs + `sandbox_identity.md` linked). Reserved terms: [`docs/concepts/terminology.md`](../concepts/terminology.md). Three phases: (1) broad sweep + categorization recorded in `output/terminology-sweep/` (working doc, not committed) [DONE - session `20260819-09`]; (2) session→iteration sweep (ops `session` words: new-session prompt+invocation, `iteration_policy.md` 61, `handover_policy.md` 47, `git_policy.md` 31, AGENTS.md 18, workflow CLI `--session`/`--session-summary`/`SESSION_NAME`/`SESSION_ARG`/`SESSION_DIR`/draft `SESSION_TS`, new devlog prose; historical records stay as-is); (3) run→session sweep (container lifecycle currently mislabeled `RUN_ID`→`SESSION_ID`: `run_id`, `agent-sandbox.run-id` label, `SESSION_STATE` write keys `run_id`, `--run-id` stop flag, compose project/volume/registry names; persistence-critical — keys+label must rename atomically, D-D back-compat decision pending). Resolves the M2.6 session-naming-collision item. Sub-decisions pending: D-A stop flag target, D-B draft `--session`→`--iteration`/`--draft`, D-C `--session-summary` target, D-D resume back-compat, D-E draft `SESSION_TS`→`ITERATION_TS`.
- [ ] **`start` command redesign (resumable/stale volume UX)** — **Design SETTLED (walk `20260818-02`): interactive-by-default wizard** — agent-run inventory first (copy via labels; bind-mount via registry), resume-N or new; freshness-on-new (implicit rebuild, auto-downgraded); config prefilled from the newest run; prints the full non-interactive command; `--run=<id>` resumes (absence = new); no subcommand split. Decision rows D6/A-Resolved/N2a/N4; implementation carried by the M2.6.6 delivery tasks
