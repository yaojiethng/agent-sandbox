...
- [ ] **Image-staleness detection — wire `STALE=image` (deferred; flag is not-yet-implemented in `20260821-08`)** — the image-staleness criterion (image `agent-sandbox.container-sig` label vs recomputed `container_sig`) is deferred to a future iteration. `make prune STALE=image` raises "not yet implemented" (`20260821-08`, D8-6) until then. Wire via `container_sig`/`_check_container_sig` in `build.sh` + `docker image inspect`; no-live-docker — test via the stub. See `docs/concepts/terminology.md` `## staleness`.
