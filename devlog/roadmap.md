...
- [x] **Image-staleness detection — wire `STALE=image` (done `20260821-09`)** — the image-staleness criterion (image `agent-sandbox.container-sig` label vs recomputed `container_sig`) implemented: `container_sig`/`_sandbox_sig_sources`/`_agent_sig_sources` lifted into `src/libs/container_sig.sh` with a shared `image_is_stale` predicate consumed by build.sh (`_check_container_sig`) and prune (`STALE=image`; `STALE=all` = sandbox OR image). Stub-tested (DOCKER_STUB_IMAGE_SIG_LABELS per-image map). See `docs/concepts/terminology.md` `## staleness`.
