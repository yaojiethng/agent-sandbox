...
- [ ] **Doc-format lint rules** -- add lint coverage for the documentation-format discipline: a rule detecting manually column-wrapped prose (hard-wrapped instruction blocks; the `MD013` line-length rule is disabled per the no-wrap policy, so the wrap has no detector). Resolves the hard-wrapped-instruction-blocks and editing-a-doc-whose-own-policy-forbids families; the plain-ASCII `doc-ascii` rule already covers non-ASCII. Progressive scope per operator ruling: (1) introduce the rule in the workflow, not live; (2) convert legacy files; (3) enable after a zero-findings confirmation. **Step 1 landed** in iteration `20260921-10`: `scripts/lint/doc-wrap.mjs` + 6 fixture tests, wired into `.markdownlint-cli2.mjs` with `doc-wrap` off and the config-driven `legacyFiles` seam tested. The tree-wide measurement ran the rule over all 593 Markdown files: zero hard-wrapped prose today, so the conversion chore (step 2) is expected empty; step 3 becomes a verify-and-flip commit.
