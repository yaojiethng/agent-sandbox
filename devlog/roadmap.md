...
- [ ] **Harness version identity (image + worktree + host) - reconciled root problem; design NEXT SESSION** - the `make resume LIST=1` slowness (`record_image_stale` -> 2xN docker inspects on identical images), the `[IMAGE_STALE]` marker / preflight warnings, the `container-sig`/`harness-sig`/`image-sig` proliferation, silent host-CLI drift, and the deferred digest-tracking are ONE root problem: no serializable, increasing version on any of the three surfaces (container image content, git worktree checkout, host installed CLI). Story `20260831-*-image_and_harness_version_identity` frames it and fixes requirements; prior solutions marked inadequate (container-sig signs only a source subset, missing base/runtime/deps; staleness is a symptom, not the defect; `make install` dispatcher execs the live checkout with no version of its own). **This session:** story + design stub + cleanup only (no design decision). Next session: settle the design mechanism (digest vs content-hash vs semver) + ADR.
