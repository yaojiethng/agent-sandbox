...
- [ ] **ADR recreation + concept-doc sweep (follow-up to `20260901-03`)** - recreate the archived ADRs under the new living per-principle format (component-scoped names chosen with operator; dated current-on-top entries; historical stack with why-rejected). Sweep `docs/concepts/` to distill content that is really ADR rationale (model-selection reasoning) into ADRs, keeping concepts current as the *what*. Re-point concept-doc links to the new stable ADR names; refresh now-stale concept content (terminology `staleness`, `sandbox_identity` image-staleness vs the version-identity ADR). Stale `docs/adr/` links now live in: `docs/architecture/security.md`, `docs/concepts/sandbox_identity.md`, `AGENTS.md`, the two skill files (`skills/improve-codebase-architecture`, `skills/domain-model` incl. a non-conforming `ADR-FORMAT.md`), and the recent handovers/discussions. Candidate first ADR: the drift/state-coherence guarantee (container-sig general model vs the minimise-not-detect redesign).
