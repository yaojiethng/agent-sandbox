...
- [ ] **ADR-policy revision (non-urgent, raised `20260901-02`)** - subsumed-design ADRs remain in `docs/adr/` alongside their superseding ones (e.g. `20260722-session_identity_and_container_markers` partial-superseded twice yet still `settled`), which is confusing + unclean to maintain. Revise `docs/operations/adr_policy.md` on how partially-subsumed ADRs are marked/handled (supersede tag on old vs deprecate-and-remove; freshness of `Status: settled` after partial supersession). Non-blocking - the `20260901` version-identity ADR was written under current policy.
