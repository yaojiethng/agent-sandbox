...
- [ ] **Testing-policy cleanup (handover `20260902-01`)** - retire the non-conforming `discovery_` prefix: rename `tests/knowledge/discovery_tar_*.sh` to `knowledge_tar_*.sh` and update the two cross-references in `docs/concepts/copy_delivery.md`; register or delete the dead test `test_list_no_sig_when_field_empty` in `tests/test_resume.sh`; add the mechanical liveness checks (every `test_*()` registered via `run_test` and every `run_test` target resolves; prerequisite liveness for the docker stub + shim). The policy edits themselves are the docs iteration `20260902-01`; this bullet is the code cleanup pass that follows approval.
- [x] **Host-side volume seed + git-enumerated tar (copy-delivery pipeline replacement)** - **done `20260901-14`.** Seed the sandbox volume host-side before the sandbox container starts (one-shot `docker compose create` + `docker cp` through the volume mount), no snapshot RO mount, gate 1/2 removed, fresh/resume compose files identical. Serialization = git-enumerated tar (`git ls-files` cached+untracked packed via `tar --null -T` under the `.agent-sandbox-seed/` sentinel prefix + `git archive HEAD` baseline member); in-container `snapshot_init_git` reconstructs index=HEAD/worktree=disk from the seeded members and repairs transform-prefixed symlink targets. Retired: `.snapshot/` staging dir, `SNAPSHOT_DIR` env/mount/substitution, `snapshot_validate`, `snapshot_dir` session-state writes, dirs.sh `SNAPSHOT_DIR` (repo-wide sweep incl. stubs + knowledge diagnostics). Deprecated `snapshot_copy_files`/`snapshot_enumerate_files` were already gone. Model: [`docs/concepts/copy_delivery.md`](../docs/concepts/copy_delivery.md); docs swept (sandbox_lifecycle, execution_model, tool_interface, security, system_overview, correspondence model). Discovery (handover `20260901-13`) validated the mechanism; the rsync negation-pattern leak is resolved by construction. **Deferred from discovery:** empty dirs not carried (git cannot represent them; accepted). Also delivered: docker-stub consolidation (`test/stubs` -> `tests/stubs/docker`, exec bit restored to 100755 after it was lost pre-baseline -- had masked 67 suite failures as rc=126) and a dry-run trace fixture project repo so the seed path is stub-tested. **Suite fully green: 758/758/0 under stub coverage.**
...
- [ ] **Seed transport redesign: helper-container copy (design settled, implementation pending)** - design settled `20260904-01` (handover `20260904-01-design-start_resume_rsync_stall.md`, design doc `20260904-design-settled-helper_container_seed.md`, ADR entry 2026-09-04). Replace the 7-step `docker cp` seed with a one-shot seeder container (project mounted read-only at `/src`, session volume at `/dest`; copy `.git` natively, stream git-enumerated tar, mixed reset to HEAD). Retires: host tar build + member-prefix transform, stdin 0B read-back verification, `baseline.tar` unpack + mixed init, rsync overlay + symlink repair, in-worktree `.agent-sandbox-seed/` staging (new requirement R7: no staging in the worktree). Includes: compose seeder wiring, `run_agent.sh` seed-path retirement, `snapshot.sh`/entrypoint cleanup, trace-test rewrites, fail-closed tripwire for repos already tracking harness state, architecture doc sweep (`execution_model.md`, `sandbox_lifecycle.md` — deferred from the design iteration).
- [x] **Documentation policy amendments (F11-F14 + STE100/skeleton-first)** - **done `20260904-02`.** Applied the operator-approved changes: `adr_policy.md` (requirements preamble, promotion cycle with failure-locus convention, sub-headers within entry fields); `documentation_policy.md` (concept docs = standalone behavioral-contract explainers, interface-level descriptions with exact commands only for external interactions, defect history only in ADR, link-sparingly reword, STE100 quick rules); `AGENTS.md` steering lines (records state not session history; skeleton-first drafting for record-layer documents) repeated in `documentation_policy.md`. Sources: findings F8-F14, handover `20260904-01`; reference example: rewritten `docs/adr/sandbox_delivery_model.md`.
