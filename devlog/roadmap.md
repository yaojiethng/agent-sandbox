...
- [ ] **Seeder stash-clear (study `20260911-study-stash_copy_prevention.md`, recommendation adopted pending impl)** - the seeder's native `.git` copy (`cp -a`) carries the host stash stack (`refs/stash`, `logs/refs/stash`) into every fresh session volume; an in-session `git stash pop` would import host WIP into the diff pipeline. Add `git stash clear` on the volume copy after the `.git` copy, plus an empty-stash tripwire in the seed self-verification. Study recommends Option A (post-copy clear) over filtered-copy prevention.
