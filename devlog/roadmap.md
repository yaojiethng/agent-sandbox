...
- [x] **Roadmap write-back completeness gate (raised 20260912, gm survey caught a stale done row the Step 7 gate missed)** - iteration_policy Step 7 item 2 now covers "completed rows the change supersedes or invalidates" (the dry-run overhaul had invalidated the "Build + dry-run interface" done row's retired staleness-gate statement without any gate step requiring its correction); corrective write-back applied to that row; AGENT_FEEDBACK 20260818-03 closed (campaign removed its last instance) (handover `20260912-07`).
- [x] **Testing-conventions rule proposals from the test-quality campaign (deferred from handover `20260912-05`)** - Anti-Pattern 7 (Test-the-Copy: never inline production source; extraction pattern + prerequisite gate); Test Structure Template made mandatory with a static runner liveness scan for dead `run_test` registrations after `test_done`; gap notes stay in function-header comments (central known-gaps file rejected); pin-citation rule added to Anti-Pattern 6; `test_common.sh` gained `assert_subshell_rc` + `source_function_from` (handover `20260912-08`).
- [x] **test_runner_selftest.sh FATAL (raised 20260912-13)** - the test-harness self-check reported `FATAL test_runner_selftest.sh: run_test registered after test_done` on every clean run since the liveness scan landed; the suite exited rc=1 despite a green report. Root cause: the scan used a naive line-based pattern and read registration-shaped words inside the selftest's own quoted payload as code. Fixed by anchoring the scan to the registration contract (`run_test` targeting a `test_` function, matching `check_test_liveness.sh`'s registration grep); the selftest Case 12 payload now writes a `test_`-named dead registration via printf so the words stay off column 0. Suite back to rc=0. (handover `20260912-15`).
- [ ] **Interface-contract compatibility (design settled 20260919-03; P0 landed 20260919-04; docs landed 20260919-05; P2 mechanism landed default-warn 20260919-06; strict/authoritative + flag removed 20260919-07; container-sig P3 pending)** - the real need behind the retired freshness signal: the host checkout driving a session and the wiring baked into the image must speak the same SHAPE (bind-mount folder shape, `SANDBOX_DIR` format, onboard command shape, host/container command semantics - the change-class-1/2/3 High band). Two contract points host-vs-container framing misses: (1) intra-session container<->container drift (agent vs sandbox images buildable independently via `--targets=agent|sandbox`); (2) session-record schema as a resume-breaking contract (`.compose/<session-id>.yml` + in-worktree `SESSION_STATE` written by host, read by both host-resume/list and container-entrypoint). **Design settled** 20260919-03: one version `INTERFACE_CONTRACT_VERSION` declared in `src/libs/interface_contract.sh`, stamped into tier-3 images at build (declaration 1) and into the record (`.compose` label set + `SESSION_STATE` key, declaration 2); comparison layered by surface: host<->container + record at start/resume preflight, container<->container at the agent entrypoint. Warn-then-strict mismatch policy with operator-released switch-over gates; container-sig rollover P0-P3, old check stripped only after the new is proven (recorded past failure: new-mechanism errors blocked start after old check stripped); doc consolidation landed 20260919-05 to one interface concept doc (`docs/concepts/sandbox_host_interface.md`, renamed from the correspondence model, 17-file propagation checklist incl. closed-handover exclusion) + one lifecycle architecture doc (`sandbox_lifecycle.md`). ADR `docs/adr/interface_contract_compatibility.md` (open — mechanism authoritative; closes on container-sig strip P3). Discussion `devlog/discussions/20260919-design-interface_contract_compatibility.md` (settled). P2: flag `interface_contract_strict()` + agent-entrypoint container↔container check landed default-warn (20260919-06). 20260919-07 removed the flag entirely: the contract is authoritative with no runtime escape hatch (an override would be a backdoor), preflight refuses on drift/missing label, the entrypoint hard-stops on a container↔container mismatch, and missing-record key/file still warns (upgrade path). P3 (strip container-sig) remains.
...
- [x] **Mount worktree with full git history (landed `20260912-14`; git-based port-back retired as a rejected alternative, verified `20260919-01`)** - full history is the default for copy and mount; `--flatten` is the opt-out (shared `snapshot_deliver` delivery dispatcher). Port-back is and remains the diff pipeline (`package_branch` / `make draft`): the export/apply path is delivery-agnostic and flatten-agnostic (verified `20260919-01` by byte-identical export artefacts and identical round-trips across full/flatten), and git-mediated port-back stays a rejected alternative (ADR `container_host_correspondence_mechanism.md`, 2026-09-19 entry). Full history buys in-tree git history for the agent, not a second port-back channel. Was "future clone strategy, not active delivery" until the verification closed the open question.
- [x] **Centralized env-precedence resolver (generated task from study `20260917-06`)** - single-source resolution for the sandbox identity triple (`PROJECT_NAME`, `PROJECT_DIR`, `SANDBOX_DIR`) with thin CLI surface, mirroring pi. Precedence per identifier: CLI flag > host env var (`AGENT_SANDBOX_*`) > per-sandbox `.env` > no runtime default; `.env` located in the provided sandbox dir, else the invocation CWD. Plan (refactor-friendly for the stale branch; prefactors before feature): **P** P4 behavioral-baseline tests FIRST (incl. a regression pinning that `.env` currently beats an explicit flag), then P1 move `PROJECT_NAME` into `.env` + drop sed injection + add `_run_refresh` insert + migration test + template bump 4->5, then P2 unified `.env` loader in new `src/libs/env.sh`, then P3 centralize base flag parsing in `common.sh` (corrected parse counts: name 7 / project 11 / sandbox 12); **S** S1 `src/libs/env_resolve.sh` resolver + unit tests, S2 wire entrypoints (start/resume, then build/stop/prune, then workflows; also fix-or-exclude the broken `package-branch` `--sandbox` forward `agent-sandbox.sh:241-248`, and provide sandbox-only resolution for resume `--list`/`--interactive`), S3 thin CLI + sandbox Makefile, S4 docs+ADR (record the `make -C` CWD contract). Decisions and thermo-nuclear review (APPROVE-WITH-CORRECTIONS, 10 corrections): handover `20260917-06`.
