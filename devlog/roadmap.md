...
- [ ] **Start/serve/dry-run interface refactor — serve as an on/off toggle on `start`; dry-run split into its own command** — operator (start `2026-08-21`), deferred beyond the start-wizard iteration (`20260821-06`) as a **major refactor**. Current surface: `start_agent.sh <mode>` with mode ∈ {standard, serve, dry-run}; `MODE` passed through to `run_agent.sh "$MODE"`; dispatcher `start`/`serve`/`dry-run` subcommands each invoke the mode; Makefile has separate `start:`/`serve:`/`dry-run:` targets with `INTERACTIVE_FLAG` only on `start:`. **Desired future shape:** (1) serve = on/off on `start` — `make start SERVE=1` (and when the wizard is wired, a serve toggle inside it); `make serve` collapses to `make start SERVE=1`; (2) dry-run = its own command, decoupled from the start wizard and serve. **Sequencing:** the serve-toggle change touches the `MODE` plumbing (`run_agent.sh "$MODE"`) and the dispatcher — land it BEFORE adding a serve toggle to the start wizard so the toggle maps to a real `--serve` flag; the dry-run split can follow independently. **Refactor-safety for `20260821-06`:** keep the wizard scoped to start mode; do not couple it to serve/dry-run invocation paths; leave `MODE` plumbing and the `serve`/`dry-run` Makefile targets and subcommands untouched so the later refactor is a clean, bounded change. Full plan in handover `20260821-06`.
...
- [ ] **Campaign findings 2026-08-21 — production fixes (operator decision needed on behavior items)** — flagged by the test-run / test-campaign / loc-reduction campaigns, pinned by tests where possible, deliberately not fixed in-campaign. Reports on the output mount (`output/test-run-20260821/`, `output/test-campaign-20260821-165041-258/`, `output/loc-reduction-20260821-175035-166983/`); ingest recorded in handovers `20260823-01`–`20260823-04`. Items:
  - [ ] **Empty `uncommitted.diff` hard-fails apply** — zero-change sessions produce exactly this artifact; needs a product decision (skip vs `--allow-empty`) before fixing; do not paper over in tests.
  - [ ] **Conventions compliance sweep** — scripts predating the rules: dual-use guards missing on `prune.sh`/`onboard.sh`/`start_agent.sh` (§1.11/§3.2; blocks unit seams, forces sed-extraction in tests); `validate_wsl_path` calls `exit` inside a library function (§3.1 return-never-exit).
  - [ ] **Error-message vs control-flow coherence** — `_session_export` calls `wait_git_lockfile` bare under `set -e`; the function prints "proceeding anyway" then returns 1, so the caller dies exactly when promised to proceed. Also fix the §4.3 `grep -c ... || echo 0` pitfall instance in `apply_run` (`0\n0` on empty diffs) and name the copyable idiom (`|| true` + `${VAR:-0}`) in bash conventions.
  - [ ] **container_sig defects** — `current_sig` memoization key ignores `repo_root` (two roots in one process → silent wrong sigs); `find|sort|xargs -0 sha256sum` degenerates to stdin read on a zero-file source set (hang hazard) — add `-r` or a stdin guard.
  - [ ] **confirm.sh savepoint rollback latent bug** — rebase-conflict rollback runs `git reset --hard confirm-savepoint` on a path where the tag was never created; a stale tag from a prior run lands the reset on the wrong commit. Behavior change — operator decision required.
  - [ ] **Naming/header contradictions (one-line doc fixes)** — `src/build/image.sh` header claims provider is lowercased but `agent_image_name` does not; compose template's self-referential `{{VAR}}` docs pollute substitution checks; `template_version` returns empty + rc0 on missing marker (unknown-version indistinguishable from success).
- [ ] **Test-harness hardening recommendations (campaigns 2026-08-21)** — stay on bash; buy the cheap gates first: assert_eq/assert_rc/assert_contains trio in `test_common.sh`; runner self-test locking the marker-counting contract; shellcheck gate over `src/ scripts/ tests/`; non-gating `bash -n` smoke target for excluded knowledge/integration/eval tests (rot detection); lib-liveness CI check asserting every `src/libs/*.sh` has a non-test source-er; dead-flag policy (boolean params only when a production call site passes them). Reconsider bats-core only if the suite grows past ~40 files or boilerplate bugs recur.
...
- [x] **Mount delivery enablement** — **consumes the compose file-set split (session `20260818-03`)**; the capability-layer entrypoint becomes delivery-aware (mount path validates `.git` + init marker, skips the snapshot gate/init, writes SESSION_STATE into the worktree `.git` as the init marker — walk N4; includes the entrypoint branch-inversion cleanup carried from `20260818-02`); `start_agent.sh` materializes the worktree via the shared snapshot primitive minus `baseline.tar` when absent and validates it before `up`; N1 worktree model (`.worktree/` default; `.snapshot` dropped from SANDBOX_DIR); `SANDBOX_TYPE`/`WORKTREE_DIR` pass through to compose; tests + docs; copy path unchanged. **Resolved — session `20260820-01`:** entrypoint delivery-aware (mount validates `.git`, writes SESSION_STATE init marker when absent, skips snapshot gate/init + `baseline.tar` preflight); `start_agent.sh` mount materialization (`snapshot_copy_worktree` minus baseline.tar + git baseline); `SANDBOX_TYPE` env as per-overlay literals (copy.yml=`copy`, mount.yml=`mount`); `execution_model.md` delivery-aware init/init-marker semantics; suite 462/0/0. Folded in this session: M2.6-close housekeeping (lines 149/241 checkboxes) + shellcheck backlog (package_branch SC2086/SC1003 + safe test fixes).
