...
- [ ] **Host-side volume seed + git-enumerated tar (copy-delivery pipeline replacement)** - promote the deferred Copy-Model Seeding task (roadmap_future, dependency landed): seed the sandbox volume host-side before `compose up` (one-shot container), no snapshot RO mount, fresh/resume compose files identical, gate 2 removed. Serialization = git-enumerated tar (`git ls-files` cached+untracked packed via `tar --null -T`). **Discovery validated `20260901-13`**: round-trip lossless (list/hashes/modes/symlinks); parity with the current pipeline across the fixture matrix; two divergences recorded - rsync negation-pattern leak (current pipeline silently copies globally-ignored files when a `!pattern` appears; git-tar correct) and empty dirs (rsync-only; git cannot represent them; accepted behavior change). Model + status: [`docs/concepts/copy_delivery.md`](../docs/concepts/copy_delivery.md). Impl iteration next: index/worktree split in-container, seed transport (`docker cp` vs `docker run --rm`), delete deprecated `snapshot_copy_files`/`snapshot_enumerate_files`, retire `.snapshot/` + gates.
