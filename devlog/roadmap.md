...
- [x] **Start/serve/dry-run interface refactor - serve as an on/off toggle on `start`; dry-run split into its own command** - **Complete (handover `20260823-16`).** Serve removed as a standalone verb (dispatcher, CLI usage, Makefile target); serve is now a toggle: `agent-sandbox start --serve` / `make start SERVE=1`. Invoking the removed verb falls through to the generic unknown-subcommand error. start_agent.sh positional modes are now standard|dry-run; internal MODE plumbing in run_agent.sh unchanged. Dry-run stays its own command; its redesign is a separate deferred entry below.
- [x] **Dry-run feature scope trim** - cut pre/post-flight to e2e-probe semantics - deferred from the command-shape refactor (handover `20260823-16`). Today dry-run pays full session-start cost before forking to the probe: identity machinery, FULL workspace snapshot (rsync + baseline archive + validate), and it persists a compose file under `.compose/<session-id>.yml` like a real session. Needed by the probe itself: image preflight, compose generation with the dry-run overlay, and the two probe scripts; the containers bind-mount `${SNAPSHOT_DIR}`, so something must exist at that path. **Options:** (a) **full trim** - lean path against a minimal placeholder snapshot dir; fastest, fully decoupled from project content, but stops exercising real-content mounts; (b) **partial trim** - keep the rsync (content-realistic), skip baseline archive + validate + session-record pollution. Decide with before/after timing measurements in the redesign iteration. **DRY constraint:** run_agent.sh's ~150-line compose-file-set assembly block must first be extracted into a lib function so any future standalone dry-run entry sources shared logic instead of copying orchestration. **Reshaped (operator, session `20260828`): renamed "feature scope trim"; re-scoped from cost-reduction to a readiness/ownership refactor.** Separation of concerns: the two dry-run containers (bearer) each run full L1..L6 self-checks and write a per-container diagnostics record; orchestration validates the correct container from the records + a hard image-signature (staleness) gate. Full repo init RETAINED (cost-trim off the table). **Delivered (handover `20260828-01`):** record contract per container (`src/libs/dry_run_record.sh` + `dry_run_image_verify`), check-ownership dedup (CP owns baked/mount/presence; dry-run owns validity/data-plane/cross/terminal), stale `brief.md` retired from probes, probes L1..L6-ordered, `make dry-run` consumes records + image-sig gate; suite 657/40/0. Design record: [`20260828-design-settled-dry_run_phase_split.md`](./discussions/20260828-design-settled-dry_run_phase_split.md).
- [ ] **Dry-run execution-point: probes at container start-up (follow-on)** - the record contract + correct-container image-sig gate are delivered; the remaining mechanism change is running the bearer checks at the containers' start-up (compose `command`/`entrypoint` override in the dry-run overlay) instead of via `docker compose exec`, reusing the full-init sequence (DRY). Deferred from `20260828-01` because it needs a start-up wrapper that re-runs init and the container-start path is unverifiable without docker. Runtime/verification concerns only -- no new assertions.
