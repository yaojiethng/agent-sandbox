...
- [ ] **Fix docker resource label reliability for prune Rule 2** - Leftover `*-sandbox-data` volumes and dangling resources on the host carry inconsistent `agent-sandbox.sandbox-dir`/`.session-id` labels, so prune's label-only Rule 2 discovery never finds them and they hang (operator observed `make prune` removing records but no resources). A host-side diagnostic `tests/knowledge/diagnose_prune_orphans.sh` (report-only, label-based) now surfaces them; a one-time `docker system prune -a --volumes -f` clears the current leftovers per operator. Next iteration: settle/relabel existing resources so Rule 2 discovery is reliable; do NOT reintroduce docker name-pattern matching (ambiguous across sandboxes; not durable under multi-`SANDBOX_DIR`). Raised `20260831-05`.
