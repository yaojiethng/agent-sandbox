...
- [ ] **Delete the remaining sed-extraction probe; source the guarded script directly in the test** - The original probe layer (`_env_field_probe`, `_template_version_probe`, `_wsl_path_probe`) was deleted with the conventions compliance sweep (`20260823-08`); one later seam remains: `template_version_probe_real` in `tests/test_onboard.sh` (added by the test-quality campaign, `20260821-13`) extracts the `template_version` body with sed and evals it in a subshell, which breaks silently on function renames. Source `scripts/onboard.sh` directly in the test and call `template_version` in-process; delete the probe. Surfaced in handover `20260901-10`; this roadmap item is the canonical record (the AGENT_FEEDBACK dual-use guards entry points here; probation).
...
- [ ] **Entrypoint map document** - the kickoff paths are undocumented: `gm` (check-in -- survey, then a menu to choose from), `new-iteration` (directive known -- gates on scope and acceptance), and the audit skills (a specific audit target). Write a document that states, per path, when to use it, what it produces, and where its output goes. Surfaced in handover `20260911-01`.
- [ ] **Architecture-doc staleness sweep** - known violation: `docs/architecture/security.md` copy row still claims "Reinitialized -- fresh `git init`, no link to host repo", but the helper-container seed (ADR 2026-09-04) copies the real host `.git` into the copy sandbox. Fix the violation, then sweep the architecture docs for the same class: behavior text that predates a landed redesign. Surfaced in handover `20260911-01`.
