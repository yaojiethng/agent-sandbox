...
- [ ] **Terminology sweep — "agent run" (harness unit) + "agent iteration" (ops unit), keyword "session" reserved** — Q9 settled (design walk `20260818-02`); own task, NOT bundled with the M2.6.6 design task. Slot 1 "run" = one container lifecycle (start→run→stop; run-id identified; creates networks/volumes/registry record; may span multiple iterations of artifacts or one draft branch); slot 2 "iteration" = work cycle producing handover + commit; both registered as STE Technical Names (definition + no bare-noun usage). Sweep: `SESSION_TS`→`RUN_TS`, `SESSION_STATE`→`RUN_STATE`, `session-diffs`→`run-diffs` (dir + compose mount target + prompts), `RESUME_SESSION`→`RESUME_RUN`, `--session`→`--run=<id>` (resolves the earlier flag-hold decision), session-ts labels, registry/volume-label fields, wizard "session step"→"run step", devlog workflow prose "session"→"iteration". Resolves the M2.6 session-naming-collision item
...
- [ ] **Mount delivery enablement** — **Depends on the compose file-set decision** (mode-selectable compose file set at generation time via `compose_generate` — this task consumes the split): `.snapshot/` mounted RW into the capability layer; agent works in `.snapshot/`; entrypoint redirect
