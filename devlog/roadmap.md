...
- [x] **Pi models-store freshness reset at container start (handover `20260912-01`)** - **done `20260912-01`.** The pi provider image bakes `models-store.json` at build time (`RUN pi install` refreshes catalogs as agentuser). Pi skips catalog network refresh while `now - checkedAt < 4h` (`REMOTE_CATALOG_REFRESH_INTERVAL_MS`, `remote-catalog-provider.js`), so a build-time-frozen store serves a stale catalog even after the `/model` refresh, and the TUI "Model catalogs refreshed." message shows regardless. Fix: pi preflight (`src/reasoning/providers/pi/preflight.sh`) sets `checkedAt: 0` on every store entry at container start, keeping `etag`/`lastModified` so the first refresh revalidates conditionally (304 if unchanged) and the baked catalog stays the offline fallback. Rejected alternatives: deleting the store (loses offline fallback, full fetch every start); `RUN pi update --models` in the dockerfile (freezes a new `checkedAt` at build time -- same staleness window relative to image age). Unit tests in `tests/test_providers_pi_preflight.sh`.
...
- [x] **Delete the remaining sed-extraction probe; source the guarded script directly in the test** - **done `20260911-04`.** The original probe layer (`_env_field_probe`, `_template_version_probe`, `_wsl_path_probe`) was deleted with the conventions compliance sweep (`20260823-08`); the last seam, `template_version_probe_real` in `tests/test_onboard.sh` (added by the test-quality campaign, `20260821-13`), extracted the `template_version` body with sed and eval'd it in a subshell. The test now calls `template_version` in-process on the shipped `Makefile.template` (handover `20260911-04`). This roadmap item was the canonical record; the AGENT_FEEDBACK dual-use guards entry pointed here (probation).
...
- [x] **Seeder stash-clear (study `20260911-study-stash_copy_prevention.md`, recommendation adopted pending impl)** - **done `20260911-08`.** The seeder runs `git stash clear` on the volume copy after the `.git` copy, with an empty-stash tripwire in the self-verification; test covers a stashed fixture (host stack untouched, volume stack empty). ADR entry 2026-09-11; handover `20260911-08`.
...
- [x] **Architecture-doc staleness sweep** - **done `20260911-05`.** Known violation fixed: `security.md` copy row claimed "fresh `git init`, no link to host repo"; the helper-container seed copies the real host `.git` natively. The sweep then covered `docs/architecture/` for the same class (behavior text predating a landed redesign): 13 verified findings fixed across `security.md`, `tool_interface.md`, `sandbox_lifecycle.md`, `execution_model.md`, `system_overview.md` -- retired-seed contract text, anonymous-volume claims, `staged.diff` artefact name, stale `libs/` paths, draft-branch pattern, duplicate `package-branch` section, mount-status wording, `providers/<n>/` path prefixes. Method comparison with the audit files recorded in [`workflow/coding-agent/audits/documentation-audit-comparison.md`](../workflow/coding-agent/audits/documentation-audit-comparison.md) (input for the M3 compiled documentation-audit prompt).
