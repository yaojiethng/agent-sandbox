...
- [ ] **Mount worktree with full git history (future clone strategy, M2.6 general - not active delivery)** - materialize the mount worktree WITH full git history (common ancestor with PROJECT_DIR -> git-based port-back becomes possible alongside the diff pipeline). The full-history mechanism landed `20260912-14`: full history is now the default for copy and mount, and `--flatten` is the flattened opt-out (shared `snapshot_deliver` delivery dispatcher). What remains future: git-based port-back itself, not the history crossing. General M2.6 sequencing note, not part of the M2.6.6 delivery task set (relocated out of M2.6.6 so that sub-milestone can compact); walk `20260818-02`.
