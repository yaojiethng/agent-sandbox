...
- [ ] **Dry-run redesign - trim pre/post-flight to e2e-probe semantics** - deferred from the command-shape refactor (handover `20260823-16`). Today dry-run pays full session-start cost before forking to the probe: identity machinery, FULL workspace snapshot (rsync + baseline archive + validate), and it persists a compose file under `.compose/<session-id>.yml` like a real session. Needed by the probe itself: image preflight, compose generation with the dry-run overlay, and the two probe scripts; the containers bind-mount `${SNAPSHOT_DIR}`, so something must exist at that path. **Options:** (a) **full diet** - lean path against a minimal placeholder snapshot dir; fastest, fully decoupled from project content, but stops exercising real-content mounts; (b) **partial diet** - keep the rsync (content-realistic), skip baseline archive + validate + session-record pollution. Decide with before/after timing measurements in the redesign iteration. **DRY constraint:** run_agent.sh's ~150-line compose-file-set assembly block must first be extracted into a lib function so any future standalone dry-run entry sources shared logic instead of copying orchestration.
