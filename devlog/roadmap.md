...
- [ ] **Test suite duration** -- reduce the total test-suite time. The suite reports ~37s wall and is wait-dominated, not CPU-bound (user ~8s, sys ~10s; measured in the lint-duration study). The test-family split landed in handover `20260920-03`. Lint and test duration are owned separately; the approach selection for the test runner is open.
