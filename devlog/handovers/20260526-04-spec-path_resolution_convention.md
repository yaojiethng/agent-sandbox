...
|---|---|---|
| `libs/*.sh` (all 15) | Every file has a self-resolution or source pattern that needs cataloguing | [x] Catalogued, 6 patterns found |
| `scripts/*.sh` (all 12) | Source paths reference libs/ files that will move | [x] Catalogued |
| `tests/test_*.sh` (all 20+) | Source paths reference ../libs/ or $REPO_ROOT/libs/ | [x] Catalogued |
| `libs/sandbox-entrypoint.sh` | Hardcoded /opt/sandbox/lib/ paths | [x] No change this session |
| `providers/pi/provider.Dockerfile` | COPY paths that must update | [x] Migrated in spec table |
| `libs/sandbox.Dockerfile` | COPY paths that must update | [x] Migrated in spec table |
| `libs/containers.sh` | build_context_* COPY paths | [x] Migrated in spec table |
| `scripts/dry_run_reasoning.sh`, `dry_run_capability.sh` | Hardcoded /opt/sandbox/lib/ paths | [x] No change this session |
