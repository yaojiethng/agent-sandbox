...
|---|---|---|---|
| 1 | `_apply_patch_file` defined in `src/libs/diff.sh` -- handles normal, FORCE, PERMISSIVE | `grep -c '_apply_patch_file' src/libs/diff.sh` >= 1 | [x] Agent |
| 2 | `apply_and_commit` defined in `src/libs/diff.sh` | `grep -c 'apply_and_commit' src/libs/diff.sh` >= 1 | [x] Agent |
| 3 | `apply_run` delegates to `_apply_patch_file` | `grep -c '_apply_patch_file' scripts/workflows/apply.sh` >= 1 | [x] Agent |
| 4 | `apply_run` without COMMIT_MSG does not commit -- existing behaviour preserved | All 9 existing apply tests pass | [x] Agent |
| 5 | `draft_run` no longer applies patches -- branch creation only | `grep 'draft_apply_patches' scripts/workflows/draft.sh` only in `_run_draft_workflow`, not in `draft_run` | [x] Agent |
| 6 | `draft_apply_patches` / `draft_apply_uncommitted` call `apply_and_commit` -- no inline git apply | `grep -c 'apply_and_commit' scripts/workflows/draft.sh` >= 2 | [x] Agent |
| 7 | `draft` CLI accepts `--force` and `--permissive` flags | Dispatch tests pass | [x] Agent |
| 8 | FORCE=true applies with `--reject`, continues on conflict | `test_apply_patch_file_force` passes | [x] Agent |
| 9 | PERMISSIVE=true retries with `--recount` | `_apply_patch_file` uses `--recount` in PERMISSIVE mode | [x] Agent |
| 10 | All unit tests pass | `bash scripts/run_tests.sh` -- 418 pass, 0 fail, 6 skipped | [x] Agent |
| 11 | Knowledge tests pass for new API | Both knowledge tests updated and pass | [x] Agent |
| 12 | Header comments describe system as built | Manual review | [p] Operator |
