...
| # | Criterion | Status |
|---|---|---|
| 1 | `scripts/dry_run_capability.sh` exists with image file existence, session state, mount, diff pipeline, and round-trip checks | [x] |
| 2 | `libs/docker-compose.dry-run.yml` mounts script into sandbox service | [x] |
| 3 | `scripts/run_agent.sh` exports `DRY_RUN_CAPABILITY_SCRIPT` | [x] |
| 4 | `libs/compose.sh` substitutes `{{DRY_RUN_CAPABILITY_SCRIPT}}` | [x] |
| 5 | `compose_dry_run` runs Phase 1 (capability) before Phase 2 (reasoning) with abort-on-failure | [x] |
| 6 | `bash -n` passes on all modified files | [x] |
| 7 | `make test` passes clean | [x] |
...
| `docs/devlog/roadmap.md` | Marked 11c as [x] |
