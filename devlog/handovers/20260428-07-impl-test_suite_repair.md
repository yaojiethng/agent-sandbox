...
| # | Criterion | Status |
|---|---|---|
| 1 | `bash tests/test_checkpoint.sh` exits 0 with all tests passed (only `worktree_id_derive` remains tested) | [x] Accepted |
| 2 | `bash tests/test_build_context.sh` exits 0 with all tests passed | [x] Accepted |
| 3 | `bash tests/test_capability_layer.sh` exits 0 (skipped with message when Docker unavailable) | [x] Accepted |
| 4 | `bash tests/test_provider_entrypoint.sh` exits 0 with 11 passed, 0 failed | [x] Accepted |
| 5 | `bash tests/test_package_diff.sh` exits 0 with all tests passed | [x] Accepted |
| 5a | `bash tests/test_package_branch.sh` exits 0 with all tests passed | [x] Accepted |
| 5b | `session_state_read` is defined in `libs/session.sh` and reads key=value pairs from `.git/SESSION_STATE` | [x] Accepted |
| 5c | `libs/package_diff.sh` sources `libs/session.sh` and uses `session_state_read` for `SESSION_TS` | [x] Accepted |
| 5d | `libs/package_branch.sh` uses `session_state_read` for `SESSION_TS` (function now exists) | [x] Accepted |
| 6 | `grep -rn '\$SNAPSHOT_DIR\|\$CHANGES_DIR\|\$INPUT_DIR\|\$OUTPUT_DIR' tests/test_*.sh` returns only local-variable definitions or read-only references -- no writes to the env-var paths | [x] Accepted |
| 7 | `grep -rn '/opt/provider-config' tests/test_*.sh` returns no results | [x] Accepted |
| 8 | All `mktemp -d` calls in `tests/test_*.sh` use an explicit `/tmp` prefix (e.g. `mktemp -d /tmp/XXXXXX`) so fixtures cannot resolve to `/opt/provider-config` | [x] Accepted |
