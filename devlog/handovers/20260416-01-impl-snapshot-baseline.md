...
|---|---|---|---|
| Untracked file | Not in index, not on disk | Copied onto disk | `?? hello-world.txt` [x] |
| Tracked file with edits | Committed version in index and on disk | Edited version overwrites disk copy | `M foo.py` [x] |
| Tracked deletion | Committed version in index and on disk | rsync `--delete` removes it from disk | `D foo.py` [x] |
| Tracked file, no changes | Committed version in index and on disk | rsync copies same content | Clean [x] |
| Gitignored file | Not in archive, not in index | Not copied by rsync | Not visible [x] |
...
|---|---|---|
| AC-SB-1 | Untracked file shows as `??` | [x] |
| AC-SB-2 | Tracked file with unstaged edits shows as `M` (unstaged), baseline contains original | [x] |
| AC-SB-3 | Tracked file with staged edits: content correct, staging state lost (expected) | [x] |
| AC-SB-4 | Tracked file deleted without staging shows as `D`, present in baseline | [x] |
| AC-SB-5 | Tracked file staged for deletion: absent from working tree, present in baseline | [x] |
| AC-SB-6 | Gitignored file absent from sandbox | [x] |
| AC-SB-7 | Clean working tree shows clean status | [x] |
| AC-SB-8 | Exactly one baseline commit, `git diff HEAD` empty | [x] |
