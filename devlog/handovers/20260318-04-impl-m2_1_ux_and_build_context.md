...
- [x] `make start` brings up two containers via `docker compose up`; capability layer starts first (service dependency)
...
- [x] `make serve` exposes port via compose override
- [x] `make dry-run` runs both containers, reasoning writes to sandbox, graceful termination, diff written
...
- [x] `make start` calls `docker build` before compose up; second `make start` with no file changes completes build step in under 5 seconds (cache hit) -- `--no-cache` removed from `build_sandbox.sh`; Docker cache handles unchanged layers
...
| `scripts/onboard.sh` produces complete working SANDBOX_DIR | All template files copied, `.workspace/` dirs created, `.env` written, `agents.md` stub produced; operator only needs to fill in `agents.md` and review `.env` before first run | `onboard.sh` |
