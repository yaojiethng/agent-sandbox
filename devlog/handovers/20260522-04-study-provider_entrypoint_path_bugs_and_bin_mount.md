...
|---|---|---|---|
| 1 | Root cause of missing skills/prompts keys identified and documented | Handover records the bug, affected code, and fix | [x] Accepted |
| 2 | `.pi/agent/bin` mount shadowing verified as active or regressed | `mount` confirms tmpfs at `bin/` | [x] Accepted |
| 3 | Both findings categorised | Handover records categories | [x] Accepted |
| 4 | AGENTS.md check warns when Pi AGENTS.md is missing | `test_warn_on_missing_agents_md` passes | [x] Accepted |
| 5 | `_ensure_harness_keys` injects skills/prompts/packages | `test_merge_adds_harness_keys` passes | [x] Accepted |
| 6 | `_ensure_harness_keys` preserves existing user keys | `test_merge_preserves_existing_keys` passes | [x] Accepted |
| 7 | `_ensure_harness_keys` deduplicates paths | `test_merge_deduplicates_paths` passes | [x] Accepted |
| 8 | Warns when settings.json not found | `test_warn_on_missing_settings` passes | [x] Accepted |
| 9 | Shared entrypoint contains no Pi-specific logic | `grep` -> 0 matches | [x] Accepted |
| 10 | Shared entrypoint sources provider preflight script if present | `grep` -> match | [x] Accepted |
| 11 | Pi Dockerfile COPIEs preflight into image | `grep` -> match | [x] Accepted |
| 12 | `containers.sh` stages preflight from providers' `preflight.sh` | `grep` -> conditional cp | [x] Accepted |
| 13 | All tests pass | Both test suites pass (5 + 8 = 13 tests) | [x] Accepted |
| 14 | Knowledge test renamed | Old file gone, new file present | [x] Accepted |
