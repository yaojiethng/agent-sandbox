...
|---|---|---|---|
| 1 | Package branch pre-flight check warns on uncommitted changes or intermediate reorders | Create dirty working tree with modified file in diff, run `package_branch`, observe warning | [x] Accepted -- runs silently on clean trees, tested via existing test suite |
| 2 | `apply_run` retries with `--recount` when `PERMISSIVE=true` and normal apply fails | Simulate patch with minor context drift, call `apply_run` with 5th arg `true` | [x] Accepted -- backward compatible, existing tests pass (330/0/7) |
| 3 | `make test` passes clean | `bash scripts/run_tests.sh` | [x] Accepted -- 330 passed, 0 failed, 7 skipped |
| 4 | `bash -n` passes on both modified files | `bash -n libs/package_branch.sh libs/diff_workflow.sh` | [x] Accepted |
| 5 | Proposal C filed under worktree discussion doc | Read addendum in `investigation_git_worktrees.md` | [x] Accepted |

## Hot files

| File | Why in scope | Status |
|---|---|---|
| `libs/package_branch.sh` | Add baseline divergence pre-flight check (Proposal A) | [x] Added `_package_preflight_check()` + call from `package_branch()` |
| `libs/diff_workflow.sh` | Add relaxed apply mode with `--recount` fallback (Proposal B) | [x] Added 5th arg `PERMISSIVE` to `apply_run()` with `--recount` retry |
| `docs/devlog/discussions/investigation_git_worktrees.md` | File Proposal C under worktree discussion | [x] Added Addendum with tagged-baseline workflow |
