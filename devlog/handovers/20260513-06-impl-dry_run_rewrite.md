...
| # | Criterion | Status |
|---|---|---|
| 1 | `dry_run.sh` no longer has capability-layer checks (sandbox-entrypoint, diff pipeline, image files) | [x] |
| 2 | `dry_run.sh` has reasoning-layer checks: identity, env vars, mounts, SESSION_STATE, cross-container marker, round-trip, stdin, liveness | [x] |
| 3 | `bash -n` passes | [x] |
| 4 | `make test` passes clean | [x] |
...
| `docs/devlog/roadmap.md` | Marked 11d as [x] |
...
---
[CORRECTION -- 2026-05-21]: `dry_run.sh` used `local _marker=...` at top-level scope (line 194), but bash only permits `local` inside a function. This caused the session-diffs round-trip check to fail with a runtime error. Fixed by removing the `local` keyword.

File changed: scripts/dry_run.sh -- removed `local` from top-level variable declaration

See also `docs/devlog/handovers/20260513-04-impl-preflight_checks.md` CORRECTION block for the related docker-compose env var fix (items 4-5), which also affected dry_run.sh path resolution.
