...
| **Image freshness signal dropped.** `[IMAGE_STALE]` / `record_image_stale` / `image_is_stale` list-time usage retired (kills the 2xN docker-inspect cost). A fresh digest is only knowable post-build, so "newer image available" is not a list-time property; the recorded digest is exact identity. No staleness column | confirmed (operator) |
...
- `record_image_stale` (`session_inventory.sh`) is the 2xN docker-inspect cost behind `resume LIST=1`.
