...
**Type:** Design -- Reframe security model from tier numbering to delivery x backing axes
**Status:** Closed

## Objective

Replace the three-tier mount model terminology (Tier 1/2/3) with a two-axis security model -- delivery (copy vs. mount) x backing (fresh baseline vs. worktree) -- ahead of the M2.6.4 design session, so that the session's ADR and architecture doc updates are written in the final vocabulary.
...
|---|---|---|
| Two-axis model (delivery x backing) replaces tier numbering | Tiers 1 and 2 are the same security posture -- the copy-vs-mount difference is delivery, not posture; the only genuine security jump is coupling to the host object store | `devlog/discussions/20260722-design-active-mount_model.md` |
