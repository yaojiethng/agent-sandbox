...
**Operator decision recorded:** option A chosen over repairing the cache  --  delete memoization entirely rather than fix the key and wire subshell-free calling (out-param convention) for a fraction-of-a-second payoff. Rationale: rule 3.3 no-speculative-infra; dead code removed beats dead code repaired.
...
| AC5 | Full suite green and deterministic x2 | 629 tests / 38 files / 0 failed x2 | accepted |
...
| Decision | Rationale |
|---|---|
| Delete memoization instead of repairing (operator-approved option A) | Inert dead code since baseline  --  subshell indirection defeats it on every production path; payoff ~0.2 s worst case; rule 3.3 |
