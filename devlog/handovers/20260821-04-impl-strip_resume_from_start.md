...
| # | Criterion | Status |
|---|---|---|
| 1 | `make start` unconditionally starts new; no `--resume` flag/branch; `_auto_resume_or_new`, `_show_volume_picker`, `_resume_from_volume`, volume-discovery removed | [x] implemented |
| 2 | `--interactive` on `start` retained as not-implemented error (start config-wizard attach point, D11) | [x] retained |
| 3 | reset-volume always forwarded (new-session-only) | [x] implemented |
| 4 | `scripts/resume_agent.sh` help Makefile forms `LIST=1`/`INTERACTIVE=1` (pre-folded item) | [x] intact |
| 5 | AGENT_FEEDBACK staleness-guard entry (pre-folded item) | [x] intact |
| 6 | Makefile template: no `RESUME`/`RESUME_FLAG` for start/serve/dry-run; resume target kept | [x] implemented |
| 7 | Docs swept (tool_interface, sandbox_lifecycle, quickstart) to two-command design | [x] implemented |
| 8 | Suite green | [x] 465/465 |
