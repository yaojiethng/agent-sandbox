...
| # | Criterion | Status |
|---|---|---|
| 1 | `make test` runs all `tests/test_*.sh` files and exits 0 when all pass, 1 when any fail | [x] Accepted |
| 2 | `make test` output names each file with its pass/fail status; the final line gives totals | [x] Accepted |
| 3 | `bash scripts/check_test_coverage.sh libs/session.sh` prints the test files that reference `session.sh`, or explicitly states none were found | [x] Accepted |
| 4 | `bash scripts/check_test_coverage.sh` with no arguments prints usage and exits 1 | [x] Accepted |
| 5 | `bash scripts/check_test_coverage.sh tests/libs/git_fixtures.sh` reports no test files found (confirms `tests/libs/` is excluded from coverage results) | [x] Accepted |
| 6 | Adding a new `tests/test_*.sh` file and running `make test` picks it up without editing the runner | [x] Accepted |
