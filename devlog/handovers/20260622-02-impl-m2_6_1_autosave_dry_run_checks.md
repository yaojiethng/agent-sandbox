...
|---|---|---|---|
| 1 | `.export-status` checked after diff pipeline test | grep for `.export-status` in `scripts/dry_run_capability.sh` | [x] |
| 2 | Autosave directory existence checked | grep for "CHANGES_DIR/autosave/" in `scripts/dry_run_capability.sh` | [x] |
| 3 | `session_export_path` resolution checked when env vars available | grep for "session_export_path" in `scripts/dry_run_capability.sh` | [x] |
| 4 | `wait_git_lockfile` checked for clean-repo path | grep for "wait_git_lockfile" in `scripts/dry_run_capability.sh` | [x] |
| 5 | `bash -n` passes on changed file | `bash -n scripts/dry_run_capability.sh` exits 0 | [x] |
| 6 | Existing tests still pass | `bash scripts/run_tests.sh` exits 0 | [x] |
