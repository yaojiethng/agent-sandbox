...
| # | Criterion | Status |
|---|---|---|
| 1 | All handovers in last 2 weeks have `**Status:**` set to `Closed` or `Active` (only current open session) | [x] Accepted |
| 2 | Correction blocks appended to all corrected handovers, referencing this handover | [x] Accepted |
| 3 | Abandoned deferred items marked `[REMOVED in M2.3]` | [x] Accepted |
| 4 | Docs restructuring investigation added to roadmap under `### Deferred (not milestone-scoped)` | [x] Accepted |
| 5 | `docs/operations/audit_policy.md` exists and covers: when to audit, scope, procedure, corrections | [x] Accepted |
| 6 | `iteration_policy.md` References table links to audit_policy.md | [x] Accepted |
| 7 | M2.3 removed from roadmap; Milestone Summary updated to `[Complete -- see changelog](changelog.md)` | [x] Accepted |
| 8 | Next sub-milestone NOT promoted -- planning deferred to next session | [x] Accepted |
| 9 | `20260505-02-impl-interactive_confirmation_flag.md` set to `Closed` | [x] Accepted |
...
| `docs/devlog/handovers/20260421-01-impl-m2_3_container_naming_labels_checkpoint.md` | Status `[x] Complete` -> `Closed`; added [CORRECTION] block |
