...
|---|---|---|---|
| 1 | `export_path` replaces both old functions; RUN_ID mandatory, LABEL optional | `bash tests/test_routing.sh` | Agent [x] |
| 2 | Session export writes to `session/<EXPORT_TIME>-<RUN_ID>/` | `bash tests/test_diff_dispatch.sh` | Agent [x] |
| 3 | Autosave writes to `autosave/<RUN_ID>/` and overwrites each cycle | `bash tests/test_diff_dispatch.sh` | Agent [x] |
| 4 | `EXPORT-TIME.txt` inside autosave dir updates each cycle | `bash tests/test_diff_dispatch.sh` | Agent [x] |
...
| 8 | No remaining references to `session_export_path` or `output_export_path` | `grep -rn` across repo | Agent [x] |
| 9 | All shell scripts pass `bash -n` | `bash -n` on every changed .sh file | Agent [x] |
