...
7. **Verify**: full suite deterministic x3; shellcheck clean; installed CLI `resume --list`/`--interactive` render (stale-CLI heuristic: diff installed `Valid subcommands` / usage after touching `scripts/agent-sandbox.sh` -- resume is dispatched from it, so re-run `make install` check).
