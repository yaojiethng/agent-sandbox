...
|---|---|---|---|
| 1 | Generated compose file contains zero `name:` lines at any indentation | `grep -c '^[[:space:]]*name:'` on compose_generate output returns 0 | Agent [x] |
| 2 | `compose_generate` strips volume-level `name:` injected by `compose config` | Test `test_no_name_lines_in_output` passes | Agent [x] |
| 3 | Compose project name is the only source of volume naming | `sha256sum` of SANDBOX_DIR is stable per directory | Agent [x] |
| 4 | `bash -n src/build/compose.sh` passes | Syntax check | Agent [x] |
| 5 | All existing trace tests still pass (21 tests) | `bash tests/test_trace_*.sh` all exit 0 | Agent [x] |
