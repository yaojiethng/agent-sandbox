...
| # | Criterion | Status |
|---|---|---|
| 1 | Root cause identified: `CHANGES_DIR_NAME=workspace/session-diffs` (subpath with '/') causes `dirs.sh` to produce `/home/agentuser/workspace/workspace/session-diffs` -- a doubled prefix | [x] |
| 2 | Fix: `CHANGES_DIR_NAME=session-diffs` (leaf-only) in compose template | [x] |
| 3 | Knowledge test exists at `tests/knowledge/knowledge_session_diffs_path_resolution.sh` with 7 assertions covering default host, container, bug reproduction, fix, mount target comparison, and leaf-name semantic check | [x] |
| 4 | Round-trip assertion added to `dry_run.sh`: checks CHANGES_DIR == mount target, writes marker, reads it back | [x] |
| 5 | **Second root cause** -- `package_branch.sh` missing from sandbox Dockerfile and build context. `diff_export` calls `source /opt/sandbox/lib/package_branch.sh` at runtime -- file didn't exist in container -> `source` failed -> `set -euo pipefail` killed autosave subshell and left empty session export directories | [x] |
| 6 | Fix: `package_branch.sh` added to `libs/sandbox.Dockerfile` COPY list and `libs/containers.sh` (`build_context_sandbox`) | [x] |
| 7 | Fix: `diff_export` now checks `package_branch` return value -- returns 1 on failure instead of silent success | [x] |
| 8 | Unit test: `test_dispatcher_missing_session_state` in `test_package_branch.sh` (regression guard, passes now) | [x] |
| 9 | Unit test: `test_diff_export_missing_session_state` in `test_diff_dispatch.sh` (failing before fix, passing after) | [x] |
| 10 | Knowledge test: `tests/knowledge/knowledge_diff_export_container.sh` -- 13 assertions covering full pipeline, untracked files, missing SESSION_STATE gap, path agreement, accumulation | [x] |
| 11 | Diagnostic script: `tests/knowledge/diagnose_autosave.sh` -- checks env vars, process state, path resolution, SESSION_STATE, live diff_export | [x] |
| 12 | `make test` passes clean (294/295, 1 pre-existing Docker skip) | [x] |
