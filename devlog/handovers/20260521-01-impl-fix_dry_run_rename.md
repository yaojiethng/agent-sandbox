...
| # | Criterion | Status |
|---|---|---|
| 1 | `scripts/dry_run_reasoning.sh` exists and `scripts/dry_run.sh` does not | [x] |
| 2 | `tests/knowledge/diagnose_dry_run_reasoning.sh` exists and `tests/knowledge/diagnose_dry_run.sh` does not | [x] |
| 3 | No `dry_run.sh` references remain in any `.sh` or `.yml` production file (doc-only references excepted) | [x] |
| 4 | `bash -n` passes on `scripts/dry_run_reasoning.sh` | [x] |
