...
| # | Criterion | Status |
|---|---|---|
| 1 | `tests/libs/git_fixtures.sh` exists with `make_committed_repo` | [x] Accepted |
| 2 | `tests/libs/session_fixtures.sh` exists with session-structure helpers | [x] Accepted |
| 3 | `test_package_branch.sh` sources `git_fixtures.sh`, no local `make_sandbox`, all tests pass | [x] Accepted |
| 4 | `test_package_diff.sh` sources `git_fixtures.sh`, no local `make_sandbox`, baseline preserved | [x] Accepted |
| 5 | `libs/session.sh` exists with `validate_project_dir` and `resolve_session_dir` per spec | [x] Accepted |
| 6 | `tests/test_session.sh` exists covering all required cases for both functions | [x] Accepted |
| 7 | `bash -n` passes on all new files; `test_session.sh` passes clean | [x] Accepted |
| 8 | `libs/draft_workflow.sh` exists with all `libs/draft.sh` functions plus `draft_run`, `confirm_run`, `reject_run` | [x] Accepted |
| 9 | `tests/test_draft_workflow.sh` exists covering full draft/confirm/reject pipeline | [x] Accepted |
| 10 | `test_draft_workflow.sh` passes clean | [x] Accepted |
| 11 | `libs/diff_workflow.sh` exists with `apply_run` matching current apply block behaviour | [x] Accepted |
| 12 | `tests/test_diff_workflow.sh` exists covering full apply pipeline | [x] Accepted |
| 13 | `test_diff_workflow.sh` passes clean | [x] Accepted |
