...
Conventions compliance sweep (roadmap "Campaign findings 2026-08-21" bullet): bring `prune.sh`, `onboard.sh`, `start_agent.sh` up to §1.11/§3.2 dual-use guard rules and `validate_wsl_path` up to §3.1 return-never-exit; delete the sed-extraction test seams those violations forced.
...
| AC5 | Suite green and deterministic ×2 | 628 tests / 38 files / 0 failed, two consecutive runs | accepted |

## Findings

| Finding | Type | Impact |
|---|---|---|
| `template_version` leaked grep's exit 1 through the pipefail pipeline on absent marker; all callers masked it via `local V=$(...)` (the §4.1 pitfall absorbing it silently). Made deterministic (`\|\| true` idiom, §4.3) — pinned contract was already rc=0+empty | latent bug (benign) | Fixed here; callers unchanged |
...
| File | Change |
|---|---|
| [`scripts/prune.sh`](../../scripts/prune.sh) | Flag parsing + validation + flow wrapped in `main()`; §1.11 guard added |
