...
| # | Criterion | Status |
|---|---|---|
| 1 | Compose template has AGENT_HOME directory bind mount + bin/ tmpfs replacing provider-config mount | [x] |
| 2 | provider-entrypoint.sh has no _copy_in/_copy_out | [x] |
| 3 | provider-entrypoint.sh has _ensure_harness_keys Node.js merge that preserves pi keys and adds harness keys | [x] |
| 4 | settings.json retains pi-managed keys after merge | [x] (tested) |
| 5 | All provider Dockerfiles have PROVIDER_CONFIG_DIR removed | [x] |
| 6 | run_agent.sh pre-creates agent/sessions | [x] |
| 7 | AGENTS.md updated in provider config | [x] |
| 8 | bash -n passes on all modified files | [x] |
| 9 | make test passes clean | [x] |
| 10 | Architecture documents describe the system as built | [x] |
