...
3x multi-line progress blocks. `--progress=quiet` was considered but rejected
...
| 2 | Thermo-nuclear review (4 findings): sig/no-sig duplication x4, filename `spinner.sh` misleading, `_run_with_progress` name hides BuildKit coupling, no TTY test coverage | all fixed; collapsed to array dispatch, renamed file+function, added unit test |
