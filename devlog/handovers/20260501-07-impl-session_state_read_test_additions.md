...
| # | Criterion | Status |
|---|---|---|
| 1 | Tree stays green | [x] `scripts/run_tests.sh` exits 0 (261 passed, 0 failed, 1 pre-existing skip) |
| 2 | `session_state_read` has 4+ test assertions | [x] `grep -c "session_state_read" tests/test_session.sh` = 14 |
| 3 | Dead env-var fallback removed from `package_diff.sh` | [x] `grep -c "\${SESSION_TS:-}" libs/package_diff.sh` = 0 |
