...
|---|---|---|
| `scripts/start_agent.sh` | `CHECKPOINT_TS` -> `SESSION_TS` rename; `BRANCH_NAME` variable for unsanitised branch | [x] Complete |
| `scripts/apply_workspace.sh` | Draft branch naming; confirm cleanup; `SESSION_TS` extraction from checkpoint tag | [x] Complete |
| `tests/test_apply_workspace.sh` | `CHECKPOINT_TS` -> `SESSION_TS`; new branch-name assertions; 2 new tests | [x] Complete |
| `tests/test_apply.sh` | `agent/draft/` -> `draft/` branch refs; session-selection test fixes | [x] Complete |
| `tests/test_start_agent.sh` | `CHECKPOINT_TS` -> `SESSION_TS` | [x] Complete |
| `libs/compose.sh` | `SESSION_NAME` format comment update | [x] Complete |
| `docs/devlog/roadmap.md` | Change 3 description updated | [x] Complete |
| `docs/architecture/sandbox_lifecycle.md` | Apply workflow section updated | [x] Complete |
| `docs/concepts/sandbox_host_correspondence_model.md` | Draft branch table row updated | [x] Complete |
| `docs/devlog/discussions/design_apply_workflow_and_baseline_advancement.md` | Draft branch format updated | [x] Complete |
| `docs/devlog/handovers/20260407-03-scope-m2_3.md` | Draft command description updated | [x] Complete |
...
| `tests/test_apply_workspace.sh` | 30x `CHECKPOINT_TS` -> `SESSION_TS`; branch refs `agent/draft/test-session` -> `draft/main-20260420-120000`; draft-state fixture updated; 2 new tests (`test_draft_branch_name_preserves_slashes`, `test_draft_branch_name_detached_head`) |
...
| `tests/test_start_agent.sh` | 18x `CHECKPOINT_TS` -> `SESSION_TS` |
