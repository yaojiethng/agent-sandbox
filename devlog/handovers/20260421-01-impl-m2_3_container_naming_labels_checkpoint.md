...
|---|---|---|
| `scripts/checkpoint.sh` | New -- checkpoint operation consolidation | [x] Complete |
| `libs/compose.sh` | Container naming + labels | [x] Complete |
| `libs/docker-compose.yml` | Container naming + labels | [x] Complete |
| `start_agent.sh` | Source checkpoint.sh; remove checkpoint-latest.ref | [x] Complete |
| `scripts/apply_workspace.sh` | Source checkpoint.sh for tag lookup; add sync | [x] Complete |
| `tests/test_checkpoint.sh` | New -- checkpoint.sh unit tests | [x] Complete |
| `tests/test_start_agent.sh` | Updated comment | [x] Complete |
| `tests/test_apply_workspace.sh` | sha1sum->sha256sum; removed ref setup | [x] Complete |
| `tests/test_apply.sh` | Worktree namespace; removed ref setup | [x] Complete |
...
---
[CORRECTION -- 2026-05-06]: Status corrected from "[x] Complete" (non-standard) to "Closed" per handover_policy.md. See 20260506-01-workflow-handover_audit_and_corrections.md.
