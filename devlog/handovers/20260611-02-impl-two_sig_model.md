...
|---|---|---|---|
| 1 | `build_image()` injects `agent-sandbox.container-sig` label | grep finds `--label "agent-sandbox.container-sig=..."` | [x] Accepted |
| 2 | `preflight()` warns on mismatch (non-blocking) | `grep "WARNING.*container-sig" scripts/build.sh` finds output | [x] Accepted |
| 3 | No hard error from container-sig check | `exit` or `return 1` not present in `_check_container_sig` | [x] Accepted |
| 4 | `sandbox_identity.md` has no stale sanbox_id references | grep empty | [x] Accepted |
| 5 | Image naming table shows project-only image names | `sandbox-<project>` without suffix | [x] Accepted |
| 6 | `sandbox_identity.md` documents container-sig | grep returns 3 matches | [x] Accepted |
| 7 | `build.sh` passes `bash -n` | `bash -n scripts/build.sh` exits 0 | [x] Accepted |
