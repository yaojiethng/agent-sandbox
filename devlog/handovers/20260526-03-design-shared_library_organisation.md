...
|---|---|---|
| Two-dimensional grouping: deployment target x lifecycle stage | Build-time files separate from runtime; host-only separate from container-deployed | Design doc Principles |
...
- The two-dimensional grid (deployment target x lifecycle stage) resolves all of them cleanly
