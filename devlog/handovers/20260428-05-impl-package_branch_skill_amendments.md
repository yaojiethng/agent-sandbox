...
| # | Criterion | Status |
|---|---|---|
| 1 | Neither skill contains "container-lifetime" or "conversation boundary" language | [x] Accepted |
| 2 | Both prompt templates include `$@` passthrough for operator elaboration | [x] Accepted |
| 3 | `package-branch.md` uses one-liner `bash ~/sandbox/libs/package_branch.sh --session-summary=...` -- no env var, no hardcoded summary | [x] Accepted |
| 4 | `package-diff.md` default invocation is bare `bash ~/sandbox/libs/package_diff.sh`; `--all` framed as optional override; no Legacy flag mention | [x] Accepted |
| 5 | Both migration-guide sections focus on change summary without scope philosophy | [x] Accepted |
| 6 | `package-branch.md` has only two main sections; make draft/confirm/reject instructions are in migration guide | [x] Accepted |
| 7 | `package_branch.sh` direct-execution mode auto-resolves from SESSION_STATE, constructs OUTPUT_DIR, accepts `--session-summary` | [x] Accepted |
