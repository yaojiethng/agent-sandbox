...
|---|---|---|---|
| 1 | `scripts/onboard.sh` contains no `sudo` calls | `grep -c sudo scripts/onboard.sh` returns 0 | Agent [x] (0) |
| 2 | `scripts/onboard.sh` has exactly two mode functions (`_run_onboard`, `_run_refresh`) with no REFRESH branching inside them | `grep -c "REFRESH" scripts/onboard.sh` <= 7 (shared prefix only) | Agent [x] (7) |
| 3 | `tests/test_onboard.sh` exists and passes with 8 tests, 0 failures | `bash tests/test_onboard.sh` exit code 0, shows "8 passed, 0 failed" | Agent [x] |
| 4 | `make test` passes (no regressions from refactoring) | `bash scripts/run_tests.sh` exit code 0 | Agent [x] (350 passed, 0 failed) |
...
| File | Reason | Status |
|---|---|---|
| `scripts/onboard.sh` | Refactored control flow (mode dispatch, sudo removal, template version colocation) | [x] Refactored |
| `tests/test_onboard.sh` | New behavioural test suite | [x] Created |
