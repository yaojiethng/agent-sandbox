...
| # | Criterion | Status |
|---|---|---|
| 1 | `testing_policy.md` contains a self-containment rule: test files source only from `tests/lib/`, never from other test files | [x] Accepted |
| 2 | `testing_policy.md` contains a "Shared Fixtures" section documenting `tests/lib/git_fixtures.sh` and `tests/lib/session_fixtures.sh` and their rules | [x] Accepted |
