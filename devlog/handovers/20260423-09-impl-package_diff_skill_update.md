...
| # | Criterion | Status |
|---|---|---|
| 1 | `libs/package_branch.sh` no longer accepts `BRANCH_NAME` as a positional arg; `SESSION_DIFFS_DIR` is the full destination path | [x] |
| 2 | `make draft` only picks up numbered diff files (`[0-9][0-9][0-9][0-9]*.diff`) | [x] |
| 3 | `tests/test_apply_workspace.sh` passes | [x] |
| 4 | `agent/prompts/package-diff.md` no longer references `patch -p1` or `.git/BASELINE_SHA` | [x] |
| 5 | `agent/prompts/package-branch.md` exists and documents the invocation pattern | [x] |
| 6 | Architecture documents in scope describe the system as built | [x] |
