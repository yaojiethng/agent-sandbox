...
- **P4 (low):** resume `--interactive` re-parses the chosen record (`record_provider` + `record_label`x2) for the confirm display, though the data is already in `RESUME_INVENTORY`.
...
6. Cross-check: full suite deterministic x3; shellcheck clean; `make install` re-run (dispatcher touched -- `Valid subcommands`/usage heuristic).
