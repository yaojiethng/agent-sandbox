...
in `make apply`, whitespace warnings from `package_diff.sh`, and file mode mismatch
...
Script fixes to `scripts/apply_workspace.sh` and `libs/package_diff.sh`. Documentation
...
- [x] `package_diff.sh` produces diffs with no trailing whitespace and exactly one trailing newline
...
|---|---|---|
| `scripts/apply_workspace.sh` | New-file pre-staging; `--index` flag; `apply` named command | [x] Complete |
| `libs/package_diff.sh` | Whitespace normalisation in diff output | [x] Complete |
| `docs/architecture/sandbox_lifecycle.md` | `checkpoint-latest.ref` reference; legacy apply description | [x] Complete |
| `docs/architecture/system_overview.md` | M1.x component descriptions; `doc-status.md` reference | [x] Complete |
| `docs/development/roadmap.md` | Change 3 description updated; `apply` naming | [x] Complete |
| `scripts/agent-sandbox.sh` | `apply` dispatch: add `apply` command arg | [x] Complete |
| `tests/test_apply_workspace.sh` | 9 no-command calls -> `apply`; header comment updated | [x] Complete |
| `tests/test_apply.sh` | "apply (legacy)" comment -> "apply command" | [x] Complete |
...
| Whitespace normalisation in `package_diff.sh` via sed pipe | `git diff` does not strip trailing whitespace from output; sed post-processing is the correct fix at the packaging side so generated diffs are clean | `package_diff.sh` |
| Standard: strip trailing whitespace per line, one trailing newline before EOF | Consistent with Linux text file conventions; matches what editors and `git apply` expect | `package_diff.sh` |
...
| `libs/package_diff.sh` | Diff generation piped through sed to strip trailing whitespace per line and ensure single trailing newline |
