...
**Type:** Housekeeping -- Roadmap deferred-item placement corrections
**Status:** Closed
...
|---|---|---|---|
| 1 | Harness-sig + staleness combined entry exists in Deferred (Unplanned); follow-on line removed from Packaging section | grep | Agent [x] |
| 2 | Process improvements entry in M3 checklist; absent from M9 | grep | Agent [x] |
| 3 | Doc Bloat subsection sits under the M3 section | grep | Agent [x] |
| 4 | Brief completed entry in M2.6.3 for docs directory restructuring | grep | Agent [x] |
| 5 | Known Issues section in `roadmap_future.md` contains the docker compose race tagged won't-fix; `roadmap.md` Deferred section removed | grep | Agent [x] |
