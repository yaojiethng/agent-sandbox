...
|---|---|---|---|
| 1 | `compose_stop` invokes `docker compose stop`, not `down` | `grep 'compose_stop' src/build/compose.sh` shows `docker compose ... stop` | Agent [x] |
| 2 | `stop.sh` no longer calls `docker rm` | `grep 'docker rm' scripts/stop.sh` returns zero | Agent [x] |
| 3 | `start_agent.sh` no longer calls `stop.sh` in pre-start guard | `grep 'stop.sh' scripts/start_agent.sh` returns zero | Agent [x] |
| 4 | `bash -n` passes on all changed files | `bash -n scripts/run_agent.sh scripts/start_agent.sh scripts/stop.sh src/build/compose.sh` | Agent [x] |
| 5 | All trace tests updated and pass (24 tests) | `bash tests/test_trace_*.sh` all exit 0 | Agent [x] |
