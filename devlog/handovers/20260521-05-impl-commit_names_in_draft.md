...
| # | Criterion | Status |
|---|---|---|
| 1 | `package_branch` produces `.msg` files alongside each `.diff` | [x] (tested via code review) |
| 2 | `make draft` reads `.msg` file content as commit message when present | [x] (`test_resolve_msg_file_used`, `test_resolve_msg_file_preferred_over_filename`) |
| 3 | `make draft` extracts subject from filename when no `.msg` file | [x] (`test_resolve_filename_subject_cleaned`) |
| 4 | Subject cleaning handles leading/trailing/consecutive underscores | [x] (`test_resolve_filename_subject_trim_underscores`) |
| 5 | `make draft` falls back to `"Apply <filename>"` when no subject available | [x] (`test_resolve_fallback_no_subject`) |
| 6 | `git-history.txt` no longer produced | [x] (removed from `package_branch.sh`) |
| 7 | All tests pass | [x] (34/34 draft, all suites 0 failures) |
