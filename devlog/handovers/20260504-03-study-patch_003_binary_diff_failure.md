...
2. **Extract `strip_index_lines`** into `libs/diff.sh` -- consolidate 7 duplicated filter call sites (6x `grep -v '^index '` + 1x inline awk) behind a single function.
...
| File | Why in scope |
|---|---|
| [`libs/diff.sh`](../../libs/diff.sh) | Added `strip_index_lines` function; replaced 2x `grep -v '^index '` |
| [`libs/package_branch.sh`](../../libs/package_branch.sh) | Replaced inline awk filter with `strip_index_lines` call |
| [`libs/draft_workflow.sh`](../../libs/draft_workflow.sh) | Replaced 2x `grep -v '^index '`; added `diff.sh` source |
| [`libs/diff_workflow.sh`](../../libs/diff_workflow.sh) | Replaced 2x `grep -v '^index '`; added `diff.sh` source |
...
| `libs/draft_workflow.sh` | Added `diff.sh` source; replaced 2x `grep -v '^index '` with `strip_index_lines` calls |
| `libs/diff_workflow.sh` | Added `diff.sh` source; replaced 2x `grep -v '^index '` with `strip_index_lines` calls |
