...
| File | Reason | Status |
|---|---|---|
| `scripts/onboard.sh` | Added `setfacl` commands for `.workspace/` and `.<provider>/` dirs | [x] |
| `providers/pi/provider.Dockerfile` | Refactored to use `$AGENT_HOME`/`$WORKSPACE_DIR`; added `chown` for workspace | [x] |
| `scripts/run_agent.sh` | Removed redundant `mkdir -p .$PROVIDER_NAME/agent/sessions` | [x] |
| `providers/pi/onboard.sh` | New file -- provider-specific setup hook | [x] |
