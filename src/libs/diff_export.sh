...
diff_export() {
  ...
  # Write .export-status with success. HEAD is the sandbox's current commit
  # at export time  --  it becomes the BASELINE for the next save's
  # session_save_needed comparison (the "nothing new since last save" check).
  local _head
  _head=$(git -C "$SANDBOX_DIR" rev-parse HEAD 2>/dev/null) || true
