...
**Rule:** Any code block in a spec that will be copy-pasted into implementation must be validated against live source within the same session.

The agent must have run a grep or read the relevant file during the session to confirm variable names, paths, and call signatures. Memory is not a substitute.
...
**Rule:** If a spec requires a structured output (coverage map, propagation table, diff summary), the format must be defined in the spec before implementation begins. An open-ended analysis requirement without a specified output format will produce inconsistent results across sessions.
...
- **Severity:** Warning — flags do not block gates but must be triaged before session close.
